...
void TotReco_En()
{
  ...
  // Double_t norm = 1.0;
  // LIK->Scale(norm/(LIK->GetIntegral()));
  // ABS->Scale(norm/(ABS->GetIntegral()));
  // WGT->Scale(norm/(WGT->GetIntegral()));
  // DAT->Scale(norm/(DAT->GetIntegral()));
  ...
  LIK->SetTitle("HGCal Energy Response (Inclusive) | Pion 200 GeV ");
  ...
  gSystem->Exec("scp PNG/RecoEn/Pion_"+energy+"_RecEn_Inc_"+DATE+".png  ngogate@lxplus7.cern.ch:"+PATH_WWW);
}
