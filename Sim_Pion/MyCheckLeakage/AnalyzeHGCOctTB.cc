...
void AnalyzeHGCOctTB::EventLoop(const char *data) {
  ...
  for (jentry=0; jentry<nentries;jentry++) {
	...
	Sum_Abs[COMP]->Fill(EABS);
	Sum_Abs_Leak[COMP]->Fill(EABS+LEAK);
	LeakFr_Abs[COMP]->Fill(EABS,LEAK/beamEnergy);

	TotSum_Abs->Fill(EABS);
	TotSum_Abs_Leak->Fill(EABS+LEAK);
	...
	RecEn[COMP]->Fill(FULL_EN);
	TotRecEn->Fill(FULL_EN);
      }
