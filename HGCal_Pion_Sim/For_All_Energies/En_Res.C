...
{
  double Find_EnRes(TH1F*,int,int,double [4][6]);
  double Res1[6],Res2[6],Res3[6],Res4[6];
  double Comp[6] = {1,2,3,4,5,6};
  double ERR[4][6];		// 4 for 4 types, 6 for 6 compartments
  double exx[6]={0};
  ...
  TFile* tf2 = new TFile("CERN_Root_Files/All_"+energy+".root");
  ...
  // for(int i=5;i<6;i++)
    {       
      ...
      Res1[i] = Find_EnRes(ABS1,0,i,ERR);
      Res2[i] = Find_EnRes(LIK1,1,i,ERR);
      Res3[i] = Find_EnRes(WGT1,2,i,ERR);
      Res4[i] = Find_EnRes(DAT1,3,i,ERR);
...
double Find_EnRes(TH1F* HH,int typ,int ii,double ERR[4][6])
{
