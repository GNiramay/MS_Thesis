...
void Reco_En()
{
  ...
  for(int i=0;i<6;i++)
      ...
      if(DoShift)
	{
      ...
      if(DoShift) LIK->SetTitle("HGCal Energy Response | Pion 200 GeV | Compartment "+HH[i]);
      else LIK->SetTitle("HGCal Energy Response (Unshifted) | Pion 200 GeV | Compartment "+HH[i]);
      ...
      if(DoShift){
      	...
      	tc->SaveAs("PNG/RecoEn/Pion_"+energy+"_RecEn_"+HH[i]+"_"+DATE+"_unshift.png");
      	tc->SaveAs("PDF/RecoEn/Pion_"+energy+"_RecEn_"+HH[i]+"_"+DATE+"_unshift.pdf");
      }
      delete tc;
    }
  if(DoShift) gSystem->Exec("scp PNG/RecoEn/Pion_"+energy+"_RecEn_"+"*_"+DATE+".png ngogate@lxplus7.cern.ch:"+PATH_WWW);
  else  gSystem->Exec("scp PNG/RecoEn/Pion_"+energy+"_RecEn_"+"*_"+DATE+"_unshift.png ngogate@lxplus7.cern.ch:"+PATH_WWW);
}
