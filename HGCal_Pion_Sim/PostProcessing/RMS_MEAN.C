...
// This time , using EnRes = RMS/Mean;
// Refer to En_Res.C for original code
...
double TotEnRes[4]={0.15,0.12,0.08,0.16};
...
void RMS_MEAN()
{
  ...
  if(DoRescale)   tmg->SetTitle("HGCal Energy resolution (After rescaling + reshifting) | Pion 200 GeV;Compartment number; Energy Resolution ( #frac{RMS}{Mean} )");
  else tmg->SetTitle("HGCal Energy resolution | Pion 200 GeV;Compartment number; Energy Resolution ( #frac{RMS}{Mean} )");
  ...
  tc->SaveAs("PNG/RecoEn/Pion_200_En_Res_"+DATE+"_RmsMean.png");
  tc->SaveAs("PDF/RecoEn/Pion_200_En_Res_"+DATE+"_RmsMean.pdf");
  gSystem->Exec("scp PNG/RecoEn/Pion_200_En_Res_"+DATE+"_RmsMean.png ngogate@lxplus7.cern.ch:"+PATH_WWW);
}
