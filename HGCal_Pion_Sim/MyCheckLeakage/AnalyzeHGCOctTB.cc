...
void AnalyzeHGCOctTB::EventLoop(const char *data) {
  ...
  Double_t E_xy[6]={0};		// <leak*abs>
  Double_t E_x[6]={0};		// <leak>
  Double_t E_y[6]={0};		// <abs>
  Double_t E_x2[6]={0};		// <leak^2>
  Double_t E_y2[6]={0};		// <abs^2>
  Double_t NComp[6]={0};	// #of events
  
  Double_t E_EEFH[6]={0};		// <leak*abs>
  Double_t E_EE[6]={0};		// <leak>
  Double_t E_FH[6]={0};		// <abs>
  Double_t E_EE2[6]={0};		// <leak^2>
  Double_t E_FH2[6]={0};		// <abs^2>

  // Double_t Wt_RecEn[6]={0.894054537326777,0.916170407695831,0.951022349025202,0.929368029739777,0.875656742556918,0.967117988394584};
  // Double_t Wt_Eabs[6]={1.39958012596221,1.37080191912269,1.33422281521014,1.30548302872063,1.28369704749679,1.31492439184747};
  // Double_t Wt_ELeak[6]={1.15673799884326,1.15673799884326,1.15340253748558,1.15473441108545,1.15340253748558,1.19260584376863};

  Double_t Wt_RecEn[6]={1};
  Double_t Wt_Eabs[6]={1};
  Double_t Wt_ELeak[6]={1};
  ...
  // Double_t Mean_RecEn=0;
  // Double_t Mean_Eabs=0;
  // Double_t Mean_ELeak=0;
  ...
  // The new lambda
  // Edited last, on 24.08.19

  float lambda[40]={0.11197, 0.145689, 0.207199, 0.240918, 0.302427, 0.336146, 0.397655, 0.431374, 0.492884, 0.526603, 0.588112, 0.621831, 0.717059, 0.778569, 0.812288, 0.873797, 0.907516, 0.969025, 1.00274, 1.07992, 1.11364, 1.19081, 1.22453, 1.29668, 1.3304, 1.40402, 1.64898, 1.70379, 1.99845, 2.2931, 2.58776, 2.88242, 3.16908, 3.57123, 3.86589, 4.16055, 4.45838, 4.74838, 5.0352, 5.13603};
  ...
  for (jentry=0; jentry<nentries;jentry++) {
    ...
    if(shower_start_index!=-1)
      {
	// Tot_Leak_Fr_EE[COMP]->Fill(rechitEnergySum_EE,(LEAK)/(beamEnergy));
	// Tot_Leak_Fr_FH[COMP]->Fill(rechitEnergySum_FH,(LEAK)/(beamEnergy));
	Tot_Leak_Fr_EE[COMP]->Fill(rechitEnergySum_EE,(energyLeakTransverseEE)/(beamEnergy));
	Tot_Leak_Fr_FH[COMP]->Fill(rechitEnergySum_FH,(energyLeakTransverseFH)/(beamEnergy));
	Sum_Abs[COMP]->Fill(EABS*Wt_Eabs[COMP]);
	Sum_Abs_Leak[COMP]->Fill((EABS+LEAK)*Wt_ELeak[COMP]);
	LeakFr_Abs[COMP]->Fill(EABS,LEAK/beamEnergy);

	TotSum_Abs->Fill(EABS*Wt_Eabs[COMP]);
	TotSum_Abs_Leak->Fill((EABS+LEAK)*Wt_ELeak[COMP]);
	h_shower_start->Fill(lambda[shower_start_index]);
	TotEn[COMP]->Fill(rechitEnergySum);
	
	TrLeakEE[COMP]->Fill(energyLeakTransverseEE);
	TrLeakFH[COMP]->Fill(energyLeakTransverseFH);
	TrLeakAH[COMP]->Fill(energyLeakTransverseAH);
	TotTrLeak[COMP]->Fill(LEAK);

	TrLeakFrEE[COMP]->Fill(energyLeakTransverseEE/beamEnergy);
	TrLeakFrFH[COMP]->Fill(energyLeakTransverseFH/beamEnergy);
	TrLeakFrAH[COMP]->Fill(energyLeakTransverseAH/beamEnergy);
	TotTrLeakFr[COMP]->Fill(LEAK/beamEnergy);
	LLeak[COMP]->Fill(energyLeakLongitudinal);
	...
	EnEE[COMP]->Fill(rechitEnergySum_EE);
	EnFH[COMP]->Fill(4.5*rechitEnergySum_FH); // beta = 4.5


	E_EEFH[COMP]+=rechitEnergySum_EE*rechitEnergySum_FH;
	E_EE[COMP]+=rechitEnergySum_EE;
	E_FH[COMP]+=rechitEnergySum_FH;
	E_EE2[COMP]+=rechitEnergySum_EE*rechitEnergySum_EE;
	E_FH2[COMP]+=rechitEnergySum_FH*rechitEnergySum_FH;

      }

    else
      {
	h_shower_start->Fill(-1);
      }
	...
	alpha = 0.40; gamma = 13.7266;
      ...
      if(shower_start_index!=-1){
	double FULL_EN = rechitEnergySum_FH/gamma + rechitEnergySumAHCAL;
	RecEn[COMP]->Fill(FULL_EN*Wt_RecEn[COMP]);
	TotRecEn->Fill(FULL_EN*Wt_RecEn[COMP]);
      }
	...
	beta = 5.5; alpha = 0.40; gamma = 73.567;
      ...
      if(shower_start_index!=-1){
	double FULL_EN = (rechitEnergySum_EE + beta*rechitEnergySum_FH)/gamma + rechitEnergySumAHCAL;
	RecEn[COMP]->Fill(FULL_EN*Wt_RecEn[COMP]);
	TotRecEn->Fill(FULL_EN*Wt_RecEn[COMP]);
      }
      // if(FULL_EN<10){cout<<"SS "<<shower_start_index<<"\tEE "<<rechitEnergySum_EE<<"\tFH "<<rechitEnergySum_FH<<"\tAH "<<rechitEnergySumAHCAL<<"\tFULL_EN "<<FULL_EN<<endl;}
  ...
  ////////// Niramay leak-eabs corr
  cout<<"Leakage-E_abs correlation:\n";
  printf("Comp\tNevents\tCovar\tRho\n");
  for(int i=0;i<6;i++){
    E_xy[i] = E_xy[i]/NComp[i];
    E_x[i] = E_x[i]/NComp[i];
    E_y[i] = E_y[i]/NComp[i];
    E_x2[i] = E_x2[i]/NComp[i];
    E_y2[i] = E_y2[i]/NComp[i];

    Double_t covar = E_xy[i]-E_x[i]*E_y[i];
    Double_t RHO = covar/sqrt((E_x2[i]-E_x[i]*E_x[i])*(E_y2[i]-E_y[i]*E_y[i]));

    // printf("comp %i\tNevents %.1f\tCovar\t%f\tRho\t%f\n",i+1,NComp[i],covar,RHO);
    printf("%i\t\t%.1f\t\t%f\t\t%f\n",i+1,NComp[i],covar,RHO);
  }

  ///////////////////////
  cout<<"\nEE-FH rechit energy correlation:\n";
  printf("Comp\tNevents\tCovar\tRho\n");
  for(int i=0;i<6;i++){
    E_EEFH[i] = E_EEFH[i]/NComp[i];
    E_EE[i] = E_EE[i]/NComp[i];
    E_FH[i] = E_FH[i]/NComp[i];
    E_EE2[i] = E_EE2[i]/NComp[i];
    E_FH2[i] = E_FH2[i]/NComp[i];

    Double_t covar = E_EEFH[i]-E_EE[i]*E_FH[i];
    Double_t RHO = covar/sqrt((E_EE2[i]-E_EE[i]*E_EE[i])*(E_FH2[i]-E_FH[i]*E_FH[i]));
    // printf("comp %i\tNevents %.1f\tCovar\t%f\tRho\t%f\n",i+1,NComp[i],covar,RHO);
    printf("%i\t\t%.1f\t\t%f\t\t%f\n",i+1,NComp[i],covar,RHO);
  }
}
