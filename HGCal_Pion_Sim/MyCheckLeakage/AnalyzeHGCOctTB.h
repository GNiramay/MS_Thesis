...
class AnalyzeHGCOctTB : public HGCNtupleVariables{
 ...
 public:
  ...
  TH2F* LeakFr_Abs[6];			// LeakFr vs Tot Abs energy
  ...
  TH1F* TotEn[6];		// Total rechit energy(EE + FH)

  TH1F* TrLeakEE[6];		// Trans. leakage in EE
  TH1F* TrLeakFH[6];		// Trans. leakage in FH
  TH1F* TrLeakAH[6];		// Trans. leakage in AH
  TH1F* TotTrLeak[6];		// Total trans leakage

  TH1F* TrLeakFrEE[6];		// Trans Leakage fraction in EE
  TH1F* TrLeakFrFH[6];		// Trans Leakage fraction in FH
  TH1F* TrLeakFrAH[6];		// Trans Leakage fraction in AH
  TH1F* TotTrLeakFr[6];		// Total trans leakage fraction
  TH1F* LLeak[6];		// Total Longitudinal Leakage

  TH1F* TotRecEn;		// Same as above graphs, but now without compartments
  TH1F* TotSum_Abs;
  TH1F* TotSum_Abs_Leak;
  TH1F* h_shower_start;		// no. of events w.r.t. shower start location

  TH1F* EnEE[6];		/* sum rechit energy in EE */
  TH1F* EnFH[6];		/* sum rechit energy in FH */
};
#endif
  ...
  for(int i=0;i<6;i++)
    {
      ...
      title = "LeakFr_Abs_";
      LeakFr_Abs[i] = new TH2F(title+HH[i],"Leakfr vs Absorber energy | SS at depth "+DD[i]+" #lambda",100,0,2*inEnergy_,100,0,0.5);
      LeakFr_Abs[i]->GetXaxis()->SetTitle("Total Absorber energy (in GeV)");
      LeakFr_Abs[i]->GetYaxis()->SetTitle("Leakage fraction = #frac{Total Transverse Leakage}{Beam Energy}");

      title = "TotEn_";
      TotEn[i] = new TH1F(title+HH[i],"Total Rechit energy sum | SS at depth "+DD[i]+"#lambda",125,0,125*inEnergy_ );
      TotEn[i]->GetXaxis()->SetTitle("Energy (in MIPs)");
      
      //////////// Leakage Abs val
      title = "TrLeakEE_";
      TrLeakEE[i] = new TH1F(title+HH[i],"Transverse leakage in EE | SS at depth "+DD[i]+"#lambda | 200 GeV Pion (sim)",160,0,inEnergy_);
      TrLeakEE[i]->GetXaxis()->SetTitle("Transverse leakage (in GeV)");

      title = "TrLeakFH_";
      TrLeakFH[i] = new TH1F(title+HH[i],"Transverse leakage in FH | SS at depth "+DD[i]+"#lambda | 200 GeV Pion (sim)",160,0,inEnergy_);
      TrLeakFH[i]->GetXaxis()->SetTitle("Transverse leakage (in GeV)");

      title = "TrLeakAH_";
      TrLeakAH[i] = new TH1F(title+HH[i],"Transverse leakage in AH | SS at depth "+DD[i]+"#lambda | 200 GeV Pion (sim)",160,0,inEnergy_);
      TrLeakAH[i]->GetXaxis()->SetTitle("Transverse leakage (in GeV)");

      title = "TotTrLeak_";
      TotTrLeak[i] = new TH1F(title+HH[i],"Total Transverse leakage | SS at depth "+DD[i]+"#lambda | 200 GeV Pion (sim)",160,0,inEnergy_);
      TotTrLeak[i]->GetXaxis()->SetTitle("Transverse leakage (in GeV)");

      title = "LLeak_";
      LLeak[i] = new TH1F(title+HH[i],"Longitudinal leakage | SS at depth "+DD[i]+"#lambda | 200 GeV Pion (sim)",160,0,inEnergy_);
      LLeak[i]->GetXaxis()->SetTitle("Leakage (in GeV)");

      //////////// Leakage Fraction

      title = "TrLeakFrEE_";
      TrLeakFrEE[i] = new TH1F(title+HH[i],"Transverse leakage fraction in EE | SS at depth "+DD[i]+"#lambda | 200 GeV Pion (sim)",150,0,0.3);
      TrLeakFrEE[i]->GetXaxis()->SetTitle("Leakage fraction = #frac{EE trans leak}{beam Energy}");

      title = "TrLeakFrFH_";
      TrLeakFrFH[i] = new TH1F(title+HH[i],"Transverse leakage fraction in FH | SS at depth "+DD[i]+"#lambda | 200 GeV Pion (sim)",150,0,0.3);
      TrLeakFrEE[i]->GetXaxis()->SetTitle("Leakage fraction = #frac{EE trans leak}{beam Energy}");

      title = "TrLeakFrAH_";
      TrLeakFrAH[i] = new TH1F(title+HH[i],"Transverse leakage fraction in AH | SS at depth "+DD[i]+"#lambda | 200 GeV Pion (sim)",150,0,0.3);
      TrLeakFrEE[i]->GetXaxis()->SetTitle("Leakage fraction = #frac{EE trans leak}{beam Energy}");

      title = "TotTrLeakFr_";
      TotTrLeakFr[i] = new TH1F(title+HH[i],"Total Transverse leakage | SS at depth "+DD[i]+"#lambda | 200 GeV Pion (sim)",150,0,0.3);
      TotTrLeakFr[i]->GetXaxis()->SetTitle("Leakage fraction = #frac{EE trans leak}{beam Energy}");      

      // 150 0 150
      title = "EnEE_";
      EnEE[i] = new TH1F(title+HH[i],"Total rechit energy in EE | SS at depth "+DD[i]+"#lambda | 200 GeV Pion (sim)",150,0,150*inEnergy_);
      EnEE[i]->GetXaxis()->SetTitle("Energy (in MIPs)");

      title = "EnFH_";
      EnFH[i] = new TH1F(title+HH[i],"Total rechit energy in FH | SS at depth "+DD[i]+"#lambda | 200 GeV Pion (sim)",150,0,150*inEnergy_);
      EnFH[i]->GetXaxis()->SetTitle("Energy (in MIPs)");

      if(i==0){
	title = "TotSum_Abs";
	TotSum_Abs = new TH1F(title,"Total E_absorber (in GeV) ", 100, 0, 2*inEnergy_ );
	TotSum_Abs->GetXaxis()->SetTitle("Total E_absorber (in GeV)");
	TotSum_Abs->GetYaxis()->SetTitle("No. of events");

	title = "TotSum_Abs_Leak";
	TotSum_Abs_Leak = new TH1F(title,"Total Sim energy (in GeV) ", 100, 0, 2*inEnergy_ );
	...
	title = "h_shower_start";
	h_shower_start = new TH1F(title,"No.of events wrt ss",10000,-2.0,5.0);
	h_shower_start->GetXaxis()->SetTitle("Shower start location (in #lambda)");
	h_shower_start->GetYaxis()->SetTitle("Number of events");
