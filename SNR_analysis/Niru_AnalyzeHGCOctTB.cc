#define Niru_AnalyzeHGCOctTB_cxx

#include <iostream>
#include <vector>
#include <cstring>
#include "Niru_AnalyzeHGCOctTB.h"
...
int main(int argc, char* argv[])
{
  ...
  Niru_AnalyzeHGCOctTB Niru_hgcOctTB(inputFileList, outFileName, data, config);
  cout << "dataset " << data << " " << endl;
  cout << "congiguration " << config << " " << endl;

  Niru_hgcOctTB.EventLoop(data);
  return 0;
}

void Niru_AnalyzeHGCOctTB::EventLoop(const char *data) {
  if (fChain == 0) return;
  ...
  for (jentry=0; jentry<nentries;jentry++) {
    ...
    if(DEBUG) cout<<"DEBUG: Start Niru_Analylizing RecHits!!"<<endl;
    ...
    for(int i = 0 ; i < NRechits; i++){
      ...
      // bool temp_mask1=(temp_layer==1 && temp_chip==0); // Niramay. temp boolean
      // if(en_chan == 3022 || en_chan == 3028 || en_chan == 44 || temp_mask1) continue; // Niramay. Mask noisy channels
      if(temp_layer<28){
	h_channel_vs_layer->Fill(temp_layer,en_chan);	     // Niramay. Filling my histogram
      }
	...
	return;
      ...
      if(module_position_ == 0 || module_position_ == 4 ) {
	...
	double h_dX = recx - (trackx);
	double h_dY = recy - (tracky);
	...
	temp = deltaR(recx,recy,trackx,tracky);
	dX = recx - (trackx);
	dY = recy - (tracky);
	...
	if(module_position_ == 0) {
	  h_rechitX_vs_rechitY_layer[temp_layer-1]->Fill(recx,recy);
	  h_trackX_vs_trackY_layer[temp_layer-1]->Fill(trackx,tracky);
	  h_rechitX_vs_trackX_layer[temp_layer-1]->Fill(recx,trackx);
	  h_rechitY_vs_trackY_layer[temp_layer-1]->Fill(recy,tracky);
	  h_dX_dY_layer[temp_layer-1]->Fill(dX,dY);
	  h_dX_layer[temp_layer-1]->Fill(h_dX);
	  h_dY_layer[temp_layer-1]->Fill(h_dY);
	  ...
	  h_trackX_vs_trackY_FH_layer[temp_layer-EE_LAYER-1][module_position_-1]->Fill(trackx,tracky);
	  h_rechitX_vs_trackX_FH_layer[temp_layer-EE_LAYER-1][module_position_-1]->Fill(recx,trackx);
	  h_rechitY_vs_trackY_FH_layer[temp_layer-EE_LAYER-1][module_position_-1]->Fill(recy,tracky);
	  ...
	  // cout<<i<<"\t"<<rechit_layer->at(i)<<"\t"<<rechit_module->at(i)<<"\t"<<temp;//<<endl;
	  //cout<<"\tFrom moduleMap : "<<temp_moduleID.at(1)<<""<<temp_moduleID.at(2)<<endl;
	  // cout<<"\tFrom moduleMap : "<<module_layer_+28<<"\t"<<module_position_<<endl;
	}
	else {
	  cout<<"ERROR: Something went wrong at Point Romeo Hotel!!!"<<endl;
	  return;
	}
      }
	...
	h_adcLG_EE[module_layer_-1][channel_bin]->Fill(rechit_amplitudeLow->at(i));
	h_TS3HG_EE[module_layer_-1][channel_bin]->Fill(rechit_TS3High->at(i));
	h_TS3LG_EE[module_layer_-1][channel_bin]->Fill(rechit_TS3Low->at(i));
      }
      if(module_part_ == 1) {
	...
	h_adcLG_FH[module_layer_-1][module_position_-1][channel_bin]->Fill(rechit_amplitudeLow->at(i));
      	h_TS3HG_FH[module_layer_-1][module_position_-1][channel_bin]->Fill(rechit_TS3High->at(i));
      	h_TS3LG_FH[module_layer_-1][module_position_-1][channel_bin]->Fill(rechit_TS3Low->at(i));
      }
	...
	// cout<<"EE Layer "<<iL+1<<"\t"<<NRechits_EE[iL]<<endl; 
      }
      else {
    	for(int j = 0; j < 7; j++) {
	  ...
	  if(j+1 == 4) {
	    ...
	    // cout<<"FH Layer "<<iL+1<<"\tPos \t"<<j<<"\t"<<NRechits_FH[iL-EE_LAYER][j]<<"\t"<<zero_rh[iL]<<endl;     
	  }
    	}
      }
    }
    ...
    for(int i=0;i< TOTAL_ACTIVE_LAYER;i++) {
      if(zero_rh[i]) continue;//if(dr_min_index[i]<0){continue;} // Niramay
      temp_moduleID.clear();
      module_position_ = -1;
      module_layer_ = -1;
      module_position_ = -1;

      // cout<<"ggg2\t"<<zero_rh[i]<<"\t"<<i+1<<endl;
      // cout<<i+1<<"\t"<<dr_min_index[i]<<"\t"<<dr_min[i]<<endl;
      int temp_layer = i;
	...
	return;
      ...
      float adcLG = rechit_amplitudeLow->at(dr_min_index[i]);
      
      if(module_position_ == 0)  {
	h_adcHG_EE_track[i][channel_bin]->Fill(adcHG);
	h_adcLG_EE_track[i][channel_bin]->Fill(adcLG);
	h_mindR[i]->Fill(dr_min[i]);
      }
      else if(module_position_ == 4) {
	h_adcHG_FH_track[i-EE_LAYER][module_position_-1][channel_bin]->Fill(adcHG);
	h_adcLG_FH_track[i-EE_LAYER][module_position_-1][channel_bin]->Fill(adcLG);
	h_mindR_FH[i-EE_LAYER][module_position_-1]->Fill(dr_min[i]);
      }
      else {
