#define Niru_HGCNtupleVariables_cxx
...
float Niru_HGCNtupleVariables::deltaR(float x1, float y1, float x2, float y2) {
  float deltaR_ = sqrt(pow((x2-x1),2) + pow((y2-y1),2));
  return deltaR_;
}


int Niru_HGCNtupleVariables::getBIN(unsigned int skiroc,unsigned int channel) {
  if (channel%2 != 0 || channel > 62 || skiroc > 3) return -1;
  else return (skiroc*32 + (channel/2));
}


std::vector<int> Niru_HGCNtupleVariables::getModuleLocation(int moduleID) {
...
std::pair<float,float> Niru_HGCNtupleVariables::dxy_alignment(int layer) {
  //int temp_find = layer;
...
//double Niru_HGCNtupleVariables::shower_comparisons(TProfile* shower, TH1F* hist) {
// TH1F* Niru_HGCNtupleVariables::shower_comparisons(TProfile* shower, TH1F* hist) {
...
// float Niru_HGCNtupleVariables::find_official_calib(int layer, int en_chan){
...
// float Niru_HGCNtupleVariables::find_my_calib(int layer, int en_chan){
//   std::pair<int, int> temp_find;
//   temp_find = std::make_pair(layer,en_chan);
