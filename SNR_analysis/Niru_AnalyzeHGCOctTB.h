#ifndef Niru_AnalyzeHGCOctTB_H
#define Niru_AnalyzeHGCOctTB_H

#include <iostream>
#include <fstream>
#include <cmath>
#include <vector>
#include "Niru_HGCNtupleVariables.h"
#include "TH1F.h"
#include "TH2F.h"
#include "TH3F.h"
#include "TFile.h"
...
class Niru_AnalyzeHGCOctTB : public Niru_HGCNtupleVariables{

 public:
  Niru_AnalyzeHGCOctTB(const TString &inputFileList="foo.txt", const char *outFileName="histo.root",const char *dataset="data", const char *config="alpha");
  ~Niru_AnalyzeHGCOctTB();
  ...
  TH1F *h_adcLG_EE[28][128];
  TH1F *h_adcLG_EE_track[28][128];
  TH1F *h_TS3HG_EE[28][128];
  TH1F *h_TS3LG_EE[28][128];
  TH1F *h_adcHG_FH[12][7][128];
  TH1F *h_adcHG_FH_track[12][7][128];
  TH1F *h_adcLG_FH[12][7][128];
  TH1F *h_adcLG_FH_track[12][7][128];

  TH1F *h_TS3HG_FH[12][7][128];
  TH1F *h_TS3LG_FH[12][7][128];
  ...
  TH2F* h_channel_vs_layer;	        // Niramay. nrechits in each channel
};
#endif

#ifdef Niru_AnalyzeHGCOctTB_cxx

void Niru_AnalyzeHGCOctTB::BookHistogram(const char *outFileName, const char* conf) {
  
  char* hname = new char[200];
  char* dir_name = new char[200];
  conf_ = conf;
  oFile = new TFile(outFileName, "recreate");
  h_channel_vs_layer = new TH2F("h_channel_vs_layer","Rechits distribution of each channel",28,0,27,2050,0,4100);
  h_channel_vs_layer->GetXaxis()->SetTitle("Layer Index");
  h_channel_vs_layer->GetYaxis()->SetTitle("Channel Index");
  h_channel_vs_layer->GetZaxis()->SetTitle("NRechits");
  ...
  for(int i = 0; i < 28; i++) {
    ...
    h_dR[i] = new TH1F(hname,hname,105.0,-1.0,20.0);


    sprintf(hname,"h_mindR_layer_%d",i+1);
    h_mindR[i] = new TH1F(hname,hname,105.0,-1.0,20.0);
    ...
    sprintf(hname,"h_dX_layer_%d",i+1);
    h_dX_layer[i] = new TH1F(hname,hname,80,-8.0,8.0);


    sprintf(hname,"h_dY_layer_%d",i+1);
    h_dY_layer[i] = new TH1F(hname,hname,80,-8.0,8.0);

    int chan = 0;
    for(int chip = 0; chip < 4; chip++) {
      for(int cc = 0; cc < 64; cc+=2,chan++) {
	sprintf(hname,"h_adcHG_EE_L%d_P0_chip%d_chan%d",i+1,chip,cc);
	h_adcHG_EE[i][chan] = new TH1F(hname, hname, 200, 0, 400);

	sprintf(hname,"h_adcLG_EE_L%d_P0_chip%d_chan%d",i+1,chip,cc);
	h_adcLG_EE[i][chan] = new TH1F(hname, hname, 20, 0, 20);

	sprintf(hname,"h_TS3HG_EE_L%d_P0_chip%d_chan%d",i+1,chip,cc);
	h_TS3HG_EE[i][chan] = new TH1F(hname, hname, 450, -50, 400);

	sprintf(hname,"h_TS3LG_EE_L%d_P0_chip%d_chan%d",i+1,chip,cc);
	h_TS3LG_EE[i][chan] = new TH1F(hname, hname, 95, -15, 80);

	sprintf(hname,"h_adcHG_EE_L%d_P0_chip%d_chan%d_track",i+1,chip,cc);
	h_adcHG_EE_track[i][chan] = new TH1F(hname, hname, 200, 0, 400);

	sprintf(hname,"h_adcLG_EE_L%d_P0_chip%d_chan%d_track",i+1,chip,cc);
	h_adcLG_EE_track[i][chan] = new TH1F(hname, hname, 20, 0, 20);
  ...
  for(int i = 0; i < 12; i++){
	...
	h_dR_FH[i][j] = new TH1F(hname,hname,105.0,-1.0,20.0);

	sprintf(hname,"h_mindR_FH_layer_L%d_P%d",i+1,j+1);
	h_mindR_FH[i][j] = new TH1F(hname,hname,105.0,-1.0,20.0);
	...
	for(int chip = 0; chip < 4; chip++) {
	  for(int cc = 0; cc < 64; cc+=2,chan++) {
	    ...
	    h_adcHG_FH[i][j][chan] = new TH1F(hname, hname, 200, 0, 400);

	    sprintf(hname,"h_adcLG_FH_L%d_P%d_chip%d_chan%d",i+1,j+1,chip,cc);
	    h_adcLG_FH[i][j][chan] = new TH1F(hname, hname, 20, 0, 20);

	    sprintf(hname,"h_TS3HG_FH_L%d_P%d_chip%d_chan%d",i+1,j+1,chip,cc);
	    h_TS3HG_FH[i][j][chan] = new TH1F(hname, hname, 450, -50, 400);

	    sprintf(hname,"h_TS3LG_FH_L%d_P%d_chip%d_chan%d",i+1,j+1,chip,cc);
	    h_TS3LG_FH[i][j][chan] = new TH1F(hname, hname, 95, -15, 80);

	    sprintf(hname,"h_adcHG_FH_L%d_P%d_chip%d_chan%d_track",i+1,j+1,chip,cc);
	    h_adcHG_FH_track[i][j][chan] = new TH1F(hname, hname, 200, 0, 400);

	    sprintf(hname,"h_adcLG_FH_L%d_P%d_chip%d_chan%d_track",i+1,j+1,chip,cc);
	    h_adcLG_FH_track[i][j][chan] = new TH1F(hname, hname, 20, 0, 20);
...
void Niru_AnalyzeHGCOctTB::Alignment_Map_Init() {
  ...
  std::cout<<"INFO: Alignment MAP initialized successfully!!!"<<endl;cout<<"L302"<<endl; // Niramay.
}


void Niru_AnalyzeHGCOctTB::moduleMap_init(const char* config) {
  char *f_name = new char[200];

  if(strcmp(config,"alpha")==0 || strcmp(config,"config1")==0) {
    sprintf(f_name,"../config_maps/moduleMAP_config1.txt");   // Niramay. Commented out for debugging purpose
  ...
  cout<<"INFO: Module Mapping Done!!! "<<endl;


}
// void Niru_AnalyzeHGCOctTB::offical_calib_init() {
//   char *f_name = new char[200];
...
// void Niru_AnalyzeHGCOctTB::my_calib_init() {
//   char *f_name = new char[200];
...
Niru_AnalyzeHGCOctTB::Niru_AnalyzeHGCOctTB(const TString &inputFileList, const char *outFileName, const char* dataset, const char* config) {
  ...
  Niru_HGCNtupleVariables::Init(tree, tree2);
  /* Niru_HGCNtupleVariables::Init(tree); */
...
Bool_t Niru_AnalyzeHGCOctTB::FillChain(TChain *chain, TChain *chain2, const TString &inputFileList) {
/* Bool_t Niru_AnalyzeHGCOctTB::FillChain(TChain *chain, const TString &inputFileList) { */
...
Long64_t Niru_AnalyzeHGCOctTB::LoadTree(Long64_t entry) {
...
Niru_AnalyzeHGCOctTB::~Niru_AnalyzeHGCOctTB() { 
