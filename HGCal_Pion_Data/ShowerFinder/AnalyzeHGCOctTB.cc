...
void AnalyzeHGCOctTB::EventLoop(const char *data) {
  ...
  int MyComp(int,float*);               // Niramay. Sorting layers into convenient compartments 
  int New_MyComp(int,float*);           // Sorting wrt layer number 
  // Double_t Wt_RecEn[6]={0.968054211035818,0.96665055582407,1.00502512562814,0.996512207274539,0.952834683182468,1.01061141990905};

  // Double_t Wt_RecEn[6]={0.970967112372937,0.969559233524643,1.00804927344849,0.999510739493018,0.955701788548112,1.01365237704017};
  Double_t Wt_RecEn[6]={1};

  for(int i=0;i<6;i++){Wt_RecEn[i]=1;}
  ...
  for (jentry=0; jentry<nentries;jentry++,hgc_jentry++) {
    ...
    for(int i = 0 ; i < ahc_nHits; i++) {
      // int temp_layer = ahc_hitK->at(i) -1; // For older pion200 ntuples
      int temp_layer = ahc_hitK->at(i); // For new pion200 ntuples
    ...
    // h_En_EEnFH->Fill((float)rechitEnergySum);

    // if(shower_start_index==1) h_en_EE2->Fill(rechitEnergySum); // Total energy when ss in 2
    // if(shower_start_index==13) h_en_EE14->Fill(rechitEnergySum); // Total energy when ss in 14
    // if(shower_start_index==27) h_en_EE28->Fill(rechitEnergySum); // Total energy when ss in 28
    // if(shower_start_index==29) h_en_FH2->Fill(rechitEnergySum);	 // Total energy when ss in 30

    /////// EE vs FH En 
    // TFile* tff = new TFile("WeightsInfo.txt");
    ifstream iff("WeightsInfo.txt");
    float ENR,AA,BB,GE,GF;		// GE:Gamma_EE GF:Gamma_FH
    float EnEE=0;			// Tot energy in EE and FH
    float EnFH=0;
    float EnAH=0;

    while(iff>>ENR>>AA>>BB>>GE>>GF){if(beamEnergy==ENR) break;}
    
    // cout<<"\t"<<ENR<<"\t"<<AA<<"\t"<<BB<<"\t"<<GE<<"\t"<<GF<<endl;
    for(int i=0;i<28;i++){EnEE+=(float)RechitsEn_EE[i];}			// EE Energy sum
    for(int i=0;i<12;i++){EnFH+=(float)RechitsEn_FH[i];}			// FH Energy sum
    for(int i=0;i<39;i++){EnAH+=(float)RechitsEn_AH[i];}			// AH Energy sum
    float WtEn=0;		// Weighted reco energy


    if(shower_start_index>-1){
      // h_En_Res[shower_start_index]->Fill((float)rechitEnergySum); // Niramay.
      // h2_EE_FH[shower_start_index]->Fill(EnEE,EnFH);    
      // if(shower_start_index<28){h_Reco_En[shower_start_index]->Fill((EnEE+BB*EnFH+AA*BB*EnAH)/GE);}
      // else{h_Reco_En[shower_start_index]->Fill((EnEE+BB*EnFH+AA*BB*EnAH)/GF);}
      if(shower_start_index<28) WtEn=(EnEE+BB*EnFH+AA*BB*EnAH)/GE;
      else WtEn=(EnFH+AA*EnAH)/GF;
      RecEn[COMP]->Fill(WtEn*Wt_RecEn[COMP]);
      TotRecEn->Fill(WtEn);
      // h_En_Res[shower_start_index]->Fill((float)(EnEE+BB*EnFH+AA*BB*EnAH));
      // else{WtEn=(EnEE+BB*EnFH+AA*BB*EnAH)/GE;} // Experimental
      // h_Reco_En[shower_start_index]->Fill(WtEn);// cout<<GE<<"\t"<<GF<<endl;
    }

    if(shower_start_index>0 && shower_start_index<26){ // Niramay
    // h_nrec_26[shower_start_index-1]->Fill(energy_sum_dR2_[26]);
    }
    if(shower_start_index>0 && shower_start_index<8){ // Niramay
      // h2_Lat_en_6[shower_start_index-1]->Fill(energy_sum_dR2_[5]);
      // h_nrec_6[shower_start_index-1]->Fill(Nrechit_layer[5]);

      for(int ii=0;ii<NRechits;ii++){
	int temp_layer = rechit_layer->at(ii);
	int temp_chip = rechit_chip->at(ii);
	int temp_channel = rechit_channel->at(ii);
	int en_chan = temp_chip*1000+temp_channel;
      
	double recx = rechit_x->at(ii);
	double recy = rechit_y->at(ii);
	...
	if(!strcmp(data,"data")) {
	  ...
	  //   continue;
    	...
    	// if(rechit_layer->at(ii)==6){ // Niramay
    	//   //h2_Lat_6[shower_start_index-1]->Fill((float)rechit_x->at(ii),(float)rechit_y->at(ii));
	//   // h2_Lat_6[shower_start_index-1]->Fill(recx,recy);
	//   double dR = deltaR(recx,recy,trackx,tracky);
	//   // if(dR < 2.0) {
	//   //   // h2_Lat_en_6[shower_start_index-1]->Fill(rechit_energy->at(ii));
	//   // }
    	// }
      }
    }
